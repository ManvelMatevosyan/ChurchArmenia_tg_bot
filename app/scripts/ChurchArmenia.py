...
def quiz(message):
    ...
    russian = telebot.types.InlineKeyboardButton("Русский🇷🇺", callback_data='russian')
    english = telebot.types.InlineKeyboardButton("English🇺🇸", callback_data='english')
    armenian = telebot.types.InlineKeyboardButton("Armenia🇦🇲", callback_data='armenia')

    markup.add(russian, english, armenian)

    bot.send_message(message.chat.id, "select the language", reply_markup=markup)
